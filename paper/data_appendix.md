...
City-level tables and overlap statistics use only rows with resolved `city` (193 projects, 39 cities in overlap table). Province-only locations in MIIT HTML (e.g. `江苏省`) remain `city=unknown` by design.
